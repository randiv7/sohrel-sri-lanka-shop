-- Fix database schema to match frontend expectations (corrected)
...
-- Fix product_images.product_id to match products.id type (both should be bigint)
...
-- we need to change product_images.product_id to bigint
-- First, let's check if there are any existing records and clear them if necessary
